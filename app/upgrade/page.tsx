...
const plans = [
  ...
  },
  {
    ...
    price: 99000,
    period: "/bulan",
    desc: "Untuk UMKM yang ingin berkembang",
    color: "border-indigo-500",
    badge: "Paling Populer",
    buttonStyle: "bg-indigo-600 text-white hover:bg-indigo-700 shadow-lg shadow-indigo-200",
    features: [
  ...
  },
  {
    ...
    price: 249000,
    period: "/bulan",
    desc: "Untuk UMKM skala menengah",
    color: "border-purple-500",
    badge: "Terlengkap",
    buttonStyle: "bg-gradient-to-r from-indigo-600 to-purple-600 text-white hover:opacity-90 shadow-lg shadow-purple-200",
    features: [
      { text: "Konsultasi AI tak terbatas", included: true },
      { text: "Kalkulator keuangan lengkap", included: true },
      { text: "Akses lowongan kerja", included: true },
...
const perks = [
  { icon: Sparkles, title: "AI Tanpa Batas", desc: "Konsultasi bisnis kapan saja tanpa batasan kuota harian", color: "bg-indigo-50 text-indigo-600" },
  ...
  { icon: Users, title: "Komunitas Eksklusif", desc: "Akses grup UMKM premium dan sesi mentoring bulanan", color: "bg-purple-50 text-purple-600" },
  { icon: Briefcase, title: "Supplier Prioritas", desc: "Koneksi langsung ke supplier terpercaya dengan harga khusus", color: "bg-orange-50 text-orange-600" },
    ...
    return (
      <div className="max-w-2xl mx-auto pt-10 pb-20 space-y-6 text-center">
        ...
        <div className="flex gap-3 justify-center">
          <Link href="/">
            <button className="bg-indigo-600 text-white px-6 py-3 rounded-xl font-bold text-sm hover:bg-indigo-700 transition-all">
              Kembali ke Beranda
            </button>
          </Link>
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-10 pb-20">
      ...
      <div className="flex items-center gap-4">
        <Link href="/profil" className="p-2 hover:bg-white rounded-full border border-gray-200 transition-all shadow-sm">
          <ArrowLeft size={20} className="text-gray-600" />
        </Link>
        <div>
          <h1 className="text-2xl font-bold text-gray-800">Upgrade Akun</h1>
          <p className="text-sm text-gray-500 mt-0.5">Pilih paket yang sesuai dengan kebutuhan bisnis kamu</p>
        </div>
      </div>

      {/* Hero */}
      <div className="relative overflow-hidden bg-gradient-to-br from-indigo-600 via-indigo-500 to-purple-600 rounded-[32px] p-8 md:p-12 text-center">
        ...
        <div className="relative">
          ...
          <p className="text-indigo-200 max-w-lg mx-auto text-base">
            Bergabung dengan 10.000+ UMKM yang sudah berkembang bersama UMKM Grow+ Premium
          </p>
          ...
          <div className="mt-6 inline-flex items-center bg-white/10 rounded-2xl p-1 gap-1">
            <button
              ...
              className={`px-5 py-2 rounded-xl text-sm font-bold transition-all ${billing === "monthly" ? "bg-white text-indigo-600 shadow-sm" : "text-white/80 hover:text-white"}`}
            >
              Bulanan
            </button>
            <button
              onClick={() => setBilling("yearly")}
              className={`px-5 py-2 rounded-xl text-sm font-bold transition-all flex items-center gap-2 ${billing === "yearly" ? "bg-white text-indigo-600 shadow-sm" : "text-white/80 hover:text-white"}`}
            >
              Tahunan
            ...
            {plan.badge && (
              <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                <span className={`px-3 py-1 text-[10px] font-bold uppercase tracking-wider rounded-full text-white ${
                  plan.id === "pro" ? "bg-indigo-600" : "bg-gradient-to-r from-indigo-600 to-purple-600"
                }`}>
                  {plan.badge}
                </span>
              </div>
      ...
      <div>
        <div className="text-center mb-6">
          ...
          <p className="text-sm text-gray-500 mt-1">Fitur-fitur eksklusif yang akan mengakselerasi bisnis kamu</p>
        </div>
      ...
      <div>
        <div className="text-center mb-6">
          ...
          <p className="text-sm text-gray-500 mt-1">UMKM yang sudah merasakan manfaatnya</p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {testimonials.map((t) => (
            <div key={t.name} className="bg-white rounded-[24px] border border-gray-100 p-5 hover:shadow-md transition-all">
              ...
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 bg-indigo-100 rounded-xl flex items-center justify-center text-indigo-600 text-xs font-bold">
                  {t.avatar}
                </div>
                <div>
      ...
      {/* CTA Bottom */}
      <div className="bg-gradient-to-br from-indigo-600 to-purple-600 rounded-[28px] p-8 text-center">
        ...
        <button
          onClick={() => router.push(`/payment/checkout?plan=pro&billing=${billing}`)}
          className="bg-white text-indigo-600 px-8 py-3 rounded-xl font-bold text-sm hover:shadow-lg hover:scale-105 transition-all inline-flex items-center gap-2">
          Coba Gratis Sekarang <ChevronRight size={16} />
        </button>
        <p className="text-indigo-300 text-xs mt-3">Tidak perlu kartu kredit</p>
      </div>
